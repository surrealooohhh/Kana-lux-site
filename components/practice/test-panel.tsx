...
export function TestPanel() {
  ...
  if (done) { const seconds = Math.round((Date.now() - start) / 1000); const accuracy = Math.round((correct / questions.length) * 100); return <div className="glass mx-auto max-w-3xl rounded-3xl p-8"><h2 className="text-3xl font-semibold">测试完成</h2><div className="mt-6 grid gap-4 sm:grid-cols-3"><Metric label="正确率" value={`${accuracy}%`} /><Metric label="错误率" value={`${100 - accuracy}%`} /><Metric label="耗时" value={`${seconds}s`} /></div><div className="mt-6 rounded-2xl bg-background/50 p-5"><h3 className="font-semibold">错误分析</h3><p className="mt-2 text-muted-foreground">{wrong.length ? wrong.join("、") : "没有错误，节奏很好。"}</p></div><Button className="mt-6" onClick={() => reset()}>重新测试</Button></div>; }
  return <div className="glass mx-auto max-w-3xl rounded-3xl p-8"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-2">{[20, 50, 100].map((size) => <Button key={size} variant={count === size ? "default" : "secondary"} onClick={() => reset(size)}>{size}题</Button>)}</div><div className="flex items-center gap-2 text-sm text-muted-foreground"><Clock className="h-4 w-4" />自动计时</div></div><Progress value={(index / questions.length) * 100} /><div className="my-8 grid place-items-center rounded-3xl bg-background/45 p-12"><div className="text-8xl font-semibold">{current?.hiragana}</div></div><div className="grid gap-3 sm:grid-cols-2">{choices.map((choice) => <Button key={choice.id} variant="secondary" size="lg" onClick={() => answer(choice.romaji)}><Target className="h-4 w-4" />{choice.romaji}</Button>)}</div></div>;
}
